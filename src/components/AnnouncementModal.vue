<template>
    <div class="modal" ref="modal" v-if="announceModalOpenProp">
        ...
        <p><input type="text" class="subjectLine" ref="emailSubject" v-model="emailSubject"
                :placeholder="defaultSubjectLine(act.actName)"></p>
        <textarea class="emailBody" ref="emailBody" v-model="emailBody"
            placeholder="Email body goes here..."></textarea>
        <p>{{emailStatus}}</p>
        <p><button @click="sendEmail" class="closeButton" ref="sendButton"
                :disabled="isDisabled(emailStatus)">SEND</button></p>
    </div>
...
export default {
    ...
    defaultSubjectLine() {
        return function (actName) {
            if (!["global", "clubs"].includes(actName)) {
